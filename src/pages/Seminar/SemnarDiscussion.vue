...
>
  ...
  <div class="gd-text-overlay1 text-h4 text-white">
    <span><strong>Time:</strong> {{ displayedDiscussion.start }} - {{ displayedDiscussion.end }}</span><br />
    <span><strong>Date:</strong> {{ displayedDiscussion.date }}</span><br />
    ...
    <template v-else>
      <div class="register-button-wrapper2" @click.stop.prevent="handleRegistration(displayedDiscussion.id)">
        <button
          style="background-color: #ffff; color: #000; border: none; font-weight: bold;"
...
const baseUrl = (process.env.VUE_APP_CORE_URL || '').replace(/\/$/g, '') + '/';
      const url = baseUrl + `api/seminars/register/${discussionId}?userEmail=${encodeURIComponent(email)}`;
    ...
    fetchDiscussions() {
       const baseUrl = (process.env.VUE_APP_CORE_URL || '').replace(/\/$/g, '') + '/';
              const fetchDiscussions = baseUrl + 'api/seminars';
