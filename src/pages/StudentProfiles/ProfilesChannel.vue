<template>
    ...
    <div class="channel">
        <div class="container" style="margin: 2%;height: 100%;">
            <div class="ChannelHeader">CHANNEL</div>
            <div v-if="!selectedGroup" class="group-list">
                ...
                <div v-for="(group, index) in displayedGroups" :key="index" @click="selectGroup(group)">
                    <div class="pro-group-item">
                        ...
                        <div v-if="group.groupName === 'Vedic Hive'" class="recentmesssage">
                            ...
                            <span class="message">
                                ...
                                {{ group.recentMessage[0]?.message?.includes("fs/download") ||
                                    group.recentMessage[0]?.message?.includes("ms/download") ? "Media" :
                                    group.recentMessage[0]?.message || "No Messages" }}
                            </span>
                        </div>
                        <span  v-if="group.groupName === 'Vedic Hive'" >
                            <button class="browse-button" @click="browse">
        ...
        <div class="grpcontainer1">
            <h4 class="grpheader">{{ header }}</h4>
            <div class="members" style="overflow-y: auto;max-height:45vh">
                <ul class="member-list">
                    <li v-for="member in grpmembers" :key="member.id" class="member-item">
                        <i class="avatar" :style="{ backgroundColor: getAvatarColor(member.username[0]) }">{{
                            member.username[0] }}</i>
...
import Chatbox from 'pages/StudentProfiles/ProfileChatBox.vue';
     ...
     browse() {
        ...
        async getUserData() {
            await this.$api.get(`api/users/${this.user.id}`).then((response) => {
                ...
                if (this.$route.path === '/StudentProfile') {
...
.pro-group-item:hover .messagesender {
    border: #B2CCFC;
}


.pro-group-item:hover {
    border: 5px solid #647dd7;
    box-shadow: 0px 0px 18px #729ecd; 
