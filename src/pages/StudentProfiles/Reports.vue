<template>
   <fin-portlet style="height: 100%;">  
    <div class="main-container">
    <div class="row-container" v-if="!isStudent">
      <div class="user-select-row">
        <!-- <label for="user-select" class="label">Users:</label> -->
        <select id="user-select" v-model="selectedUser" @change="filterVMsByUser" class="user-select">
...
import DoughnutChart2 from '../reports/components/Dummy/DoughnutChart2.vue';
...
.main-container {
  ...
  margin: 0 auto; /* Center alignment without additional top margin */
...
.row-container {
  ...
  border-radius: 8px;
  /* padding: 1rem;s */
  margin: 0; /* Remove any margin if added accidentally */
}
...
.label {
  display: block;
  margin-bottom: 0.5rem; /* Maintain uniform spacing */
  font-weight: bold;
}
...
.chart-container {
  ...
  margin-top: 0; /* Ensure no top margin is applied */
}
...
.navigation-buttons {
  display: flex;
  justify-content: space-between;
  margin-top: 10px; /* Consistent margin spacing */
}
