<template>
  ...
  <div class="col-12 row" style="width:100vw">
              ...
              <q-td auto-width class="text-right">
                <q-btn
                  ...
                  @click="() => { props.expand = !props.expand;
                     fetchStudentAssignments(props.row.studentId);
                   }"
...
async fetchBatches() {
  try {
    const baseUrl = (process.env.VUE_APP_CORE_URL || '').replace(/\/$/g, '') + '/';
    const batchUrl = `${baseUrl}api/accounts`;
    ...
    if (data.success) {
      ...
      this.batches = data.data
        .filter(batch => ![1002, 1008].includes(batch.id))
        .map(batch => {
          let cycleid_new;

          if (String(batch.id).startsWith('0')) {
            cycleid_new = batch.id === '0001'
              ? `24A${batch.cycleid?.slice(-3) || ''}`
              : `24A${batch.cycleid || ''}`;
          } else {
            cycleid_new = `90A${batch.id}`;
          }
      ...
      // ✅ Use user accountId as the preferred batch
      const userAccountId = this.user?.accountId;
      console.log("User Account ID:", userAccountId);

      if (userAccountId) {
        console.log("Using user's account batch:", userAccountId);
        await this.fetchAssignments(userAccountId);
        await this.fetchStudents(userAccountId);
        console.log("Assignments and students fetched for user's account batch:", userAccountId);
      } else if (this.filteredBatches.length > 0) {
        const fallbackBatch = this.filteredBatches[0];
        console.warn(`User's accountId (${userAccountId}) not in filtered list. Falling back to first batch:`, fallbackBatch);
        await this.fetchAssignments(fallbackBatch.id);
        await this.fetchStudents(fallbackBatch.id);
      } else {
        console.warn('No valid batches found after filtering.');
      }
    } else {
  ...
  try {
    const baseUrl = (process.env.VUE_APP_CORE_URL || '').replace(/\/$/g, '') + '/';
    const studentsUrl = `${baseUrl}api/enrollments/account/${cycleid}`;
...
async fetchAssignments(cycleid) {
  try {
    const baseUrl = (process.env.VUE_APP_CORE_URL || '').replace(/\/$/g, '') + '/';
    const assignmentUrl = `${baseUrl}api/enrollmentsAssignments/by-account?accountId=${cycleid}`;
...
closeDialog() {
 ...
 async fetchStudentAssignments(studentId) {
  try {
    const baseUrl = (process.env.VUE_APP_CORE_URL || '').replace(/\/$/g, '') + '/';
    const studentAssignmentUrl = `${baseUrl}api/student-assignments/hackathon?&studentId=${studentId}`;
    console.log("Fetching student assignments from URL:", studentAssignmentUrl);
    ...
    if (response.data && response.data.success && response.data.data) {
      const assignmentsWithBlobs = await Promise.all(
        response.data.data.map((assignment, index) => {
          ...
          // Use assignment.batchId here, NOT batchId (which was undefined)
          const batchId_new = assignment.batchId.startsWith('0')
            ? assignment.batchId === '0001'
              ? `24A${assignment.batchId.slice(-3)}`  // Remove an extra zero for 0001
              : `24A${assignment.batchId}`
            : `90A${assignment.batchId}`;
      ...
      // Example: find selectedRow based on studentId only, or batchId if you want
      // Adjust condition based on your data structure
